// AutoVFX ExtendScript for Premiere Pro
// This file contains functions that run within Premiere Pro's ExtendScript engine

addJson()

// Global namespace for AutoVFX ExtendScript functions
var AutoVFXExtendScript = (function() {
    
    /**
     * Export sequence using Premiere Pro native export with .epr preset
     */

    function exportSequenceWithPreset(outputPath, scriptLocation, useRunwayOptimization) {
        var ret = { success: false, outputPath: outputPath, inPoint: null, outPoint: null, parentTrack: null };
        var expSuccess = false;
        var ttqq = new Time();
        ttqq.seconds = -400000;
        var bkpInOut = {inPoint: ttqq, outPoint: ttqq};

        try {
            // Check if there's an active sequence
            if (!app.project.activeSequence) {
                ret.error = "No active sequence found";
                return JSON.lave(ret);
            }

            var sequence = app.project.activeSequence;
            
            // Check for in/out points, backup them to restore after export
            var inPoint, outPoint;
            inPoint = sequence.getInPointAsTime();
            outPoint = sequence.getOutPointAsTime();

            // Always backup the current in/out points regardless of their values
            bkpInOut.inPoint = inPoint;
            bkpInOut.outPoint = outPoint;

            //find the first video in selection
            var vidSel, seqSel = app.project.activeSequence.getSelection();
            for(var i = 0; i< seqSel.length; i++){
                if(seqSel[i].mediaType == "Video") {
                    vidSel = seqSel[i];
                    break;
                }
            }
            // Check selected clip
            if (!vidSel) {
                ret.error = "No selected video clip found";
                return JSON.lave(ret);
            }

            // Check if there's an active sequence
            if (vidSel.duration.seconds > 10) {
                ret.error = "Selected range is too long. Maximum export duration is 10 seconds.";
                return JSON.lave(ret);
            }

            // Ensure minimum duration for video processing (1 second minimum)
            var minDurationSeconds = 1.0;
            var actualDuration = vidSel.duration.seconds;
            
            // Set in/out points based on the selected clip
            // This ensures we export ONLY the selected clip, not the entire sequence
            var clipInPoint = vidSel.inPoint;
            var clipOutPoint = vidSel.outPoint;
            
            sequence.setInPoint(clipInPoint);
            
            if (actualDuration < minDurationSeconds) {
                // Extend the out point to meet minimum duration requirement
                var extendedOutPoint = new Time();
                extendedOutPoint.ticks = clipInPoint.ticks + (minDurationSeconds * sequence.timebase);
                
                // Make sure we don't extend beyond the clip's actual end
                var clipEndPoint = clipInPoint.ticks + vidSel.duration.ticks;
                if (extendedOutPoint.ticks > clipEndPoint) {
                    // If the clip itself is too short, we need to extend it or show an error
                    ret.error = "Selected clip is too short (" + actualDuration.toFixed(2) + "s). Video processing requires clips to be at least 1 second long. Please select a longer clip or extend your clip in the timeline.";
                    return JSON.lave(ret);
                }
                
                sequence.setOutPoint(extendedOutPoint);
            } else {
                sequence.setOutPoint(clipOutPoint);
            }
            
            // Double-check: Verify the in/out points are correctly set for export
            var exportInPoint = sequence.getInPointAsTime();
            var exportOutPoint = sequence.getOutPointAsTime();
            
            // Store the actual export range in return object for debugging
            ret.exportInPoint = exportInPoint.ticks;
            ret.exportOutPoint = exportOutPoint.ticks;

            //mute tracks up to clip
            var trks = app.project.activeSequence.videoTracks;
            var hideTrks = [];
            for(var i = vidSel.parentTrackIndex+1; i< trks.length; i++){
                if(!trks[i].isMuted()){ 
                    trks[i].setMute(1);
                    hideTrks.push(trks[i]);
                }
            }

            // Call the native export function
            var presetFileName = "AutoVFX_export.epr"; // Default
            
            if (useRunwayOptimization) {
                // Check sequence format and choose appropriate preset
                var sequence = app.project.activeSequence;
                var is4K = sequence.frameSizeHorizontal >= 3840 || sequence.frameSizeVertical >= 2160;
                
                if (is4K) {
                    presetFileName = "AutoVFX_runway_4k.epr";
                } else {
                    presetFileName = "AutoVFX_runway_16mb.epr";
                }
            }
            
            var presetPath = scriptLocation + "/" + presetFileName;
            
            // Check if the optimized preset exists, fallback to default if not
            var presetFile = new File(presetPath);
            if (useRunwayOptimization && !presetFile.exists) {
                presetFileName = "AutoVFX_export.epr";
                presetPath = scriptLocation + "/" + presetFileName;
            }
            
            // if windows replase slash to backslash (needs only for exportAsMediaDirect()) 
            var tryFold = new File(outputPath);
            tryFold = tryFold.parent;
            if(!tryFold.exists) tryFold.create();
            if($.os.indexOf("Windows")>-1){ outputPath = outputPath.split("/").join("\\"); presetPath = presetPath.split("/").join("\\"); }
            expSuccess = app.project.activeSequence.exportAsMediaDirect(outputPath, presetPath, app.encoder.ENCODE_IN_TO_OUT);
            
            // return to the state before export
            for(var i = 0; i< hideTrks.length; i++){ hideTrks[i].setMute(0); }
            
            // Properly restore the original in/out points
            if(bkpInOut.inPoint && bkpInOut.inPoint.seconds > -400000) {
                sequence.setInPoint(bkpInOut.inPoint);
            }
            if(bkpInOut.outPoint && bkpInOut.outPoint.seconds > -400000) {
                sequence.setOutPoint(bkpInOut.outPoint);
            }
            ret.inPoint = vidSel.inPoint.ticks;
            ret.outPoint = vidSel.outPoint.ticks;
            ret.parentTrack = vidSel.parentTrackIndex;
            ret.timebase = sequence.timebase;
        } catch (error) { ret.error = "Exception during export: " + error.toString(); }

        if (expSuccess) { ret.success = true; } 
        else { ret.error = "Export failed - exportAsMediaDirect returned false"; }
        return JSON.lave(ret);
    }
     ...
     */
    // importData = { outputPath: exportData.outputPath, clipStart: exportData.inPoint,
    //     clipEnd: exportData.outPoint, parentTrack: exportData.parentTrack, timebase: exportData.timebase };
    function importVideoToTimeline(importData, insertAtPlayhead) {
        try {
            //Check seq
            if (!app.project.activeSequence) { return JSON.lave({ success: false, error: "No active sequence found" }); }
            
            // Create a new project item from the video file
            var importSuccess = app.project.importFiles([importData.outputPath]);
            if (!importSuccess) { return JSON.lave({ success: false, error: "Failed to import video file" }); }
            
            // Find the imported item
            var impItem = null;
            var expFile = new File(importData.outputPath);
            for (var i = app.project.rootItem.children.numItems - 1; i >= 0; i--) {
                var item = app.project.rootItem.children[i];
                if (item.name === expFile.displayName) {
                    impItem = item;
                    break;
                }
            }
            if (!impItem) { return JSON.lave({ success: false, error: "Could not find imported video in project" }); }
            impItem.setScaleToFrameSize()

            var sequence = app.project.activeSequence;
            
            // Find the topmost empty video track or create a new one
            var videoTrack = null;
            var targetTrackIndex = -1;
            
            // Look for an empty track from top to bottom
            
            for (var trackIdx = importData.parentTrack+1; trackIdx < sequence.videoTracks.numTracks -1; trackIdx++) {
                if (sequence.videoTracks[trackIdx].clips.numItems === 0) {
                    videoTrack = sequence.videoTracks[trackIdx];
                    targetTrackIndex = trackIdx;
                    break;
                }
            }
            
            // If no empty track found, use the topmost track (but this might overlay existing content)
            if (!videoTrack) {
                // Try to add a new track 
                try {
                    app.enableQE();
                    var seqQE = qe.project.getActiveSequence();
                    var newTrInd = seqQE.numVideoTracks;
                    seqQE.addTracks(1, newTrInd, 0);
                    videoTrack = sequence.videoTracks[sequence.videoTracks.numTracks - 1];
                } catch (e) {
                    // Fall back to topmost existing track
                    videoTrack = sequence.videoTracks[sequence.videoTracks.numTracks - 1];
            ...
            var insertionTime, msg = "Video imported successfully";
            // Use the original clip's start position on the timeline
            try { if (importData.inPoint) { 
                insertionTime = new Time();
                insertionTime.ticks = importData.inPoint; } 
            } catch (e) { insertionTime = undefined; }

            if (insertAtPlayhead || !insertionTime) { insertionTime = sequence.getPlayerPosition(); } 
            
            // Insert the clip
            var newClip;
            try { 
                newClip = videoTrack.insertClip(impItem, insertionTime.ticks); 
                if(newClip){
                    newClip = undefined;
                    var viTrClips = videoTrack.clips;
                    var inT = insertionTime.seconds+1;
                    for(var i = 0; i< viTrClips.length; i++){
                        if(viTrClips[i].start.seconds < inT && viTrClips[i].end.seconds > inT){
                            newClip = viTrClips[i]; break; }
                    }
                }
            } 
            catch (error) {
                insertionTime = sequence.getPlayerPosition();
                newClip = videoTrack.insertClip(impItem, insertionTime.ticks);
                msg = "Failed to parse position info. Video imported by player position: " + error.toString();
            }
            ...
            if (newClip) {
                return JSON.lave({
                    success: true,
                    message: msg,
                    insertionTime: insertionTime.seconds,
                    clipDuration: newClip ? newClip.duration:"null",
                    trackIndex: targetTrackIndex,
                    trackInfo: targetTrackIndex === sequence.videoTracks.numTracks - 1 ? "topmost track" : "empty track " + (targetTrackIndex + 1)
                });
            } else {
                return JSON.lave({
                    success: false,
                    error: "Failed to insert clip into timeline"
                });
            }
            ...
            return JSON.lave({
                success: false,
                error: "Exception during import: " + error.toString()
            });
        }
    }

    function exportSequenceAE(outputPath){
        var ret = { success: false, outputPath: outputPath, 
            inPoint: null, outPoint: null, parentTrack: null };
        var bkpInOut = {inPoint: 0, outPoint: 0};
        try {
            if(!app.project){
                ret.error = "No active project found";
                return JSON.lave(ret);
            }

            var appProjQue = app.project.renderQueue;
            var appProjAct = app.project.activeItem;
            if(!appProjAct || appProjAct.typeName != "Composition"){
                // try to get from selection ?
                ret.error = "No active composition found";
                return JSON.lave(ret);
            }

            var prjSelLay = appProjAct.selectedLayers;
            if(!prjSelLay || prjSelLay.length < 1){
                ret.error = "No active selected layer found";
                return JSON.lave(ret);
            }
            else prjSelLay = prjSelLay[0];

            bkpInOut.inPoint = prjSelLay.inPoint;
            bkpInOut.outPoint = prjSelLay.outPoint;
            if (bkpInOut.outPoint - bkpInOut.inPoint > 10) {
                ret.error = "Selected range is too long. Maximum export duration is 10 seconds.";
                return JSON.lave(ret);
            }

            //hide other, set work area
            var saveSoloState = prjSelLay.solo
            prjSelLay.solo = true;
            var saveArStrt = appProjAct.workAreaStart;
            var saveArDrtn = appProjAct.workAreaDuration;
            appProjAct.workAreaStart = prjSelLay.inPoint;
            appProjAct.workAreaDuration = prjSelLay.outPoint - prjSelLay.inPoint;

            // clear render queue
            for(var i = appProjQue.numItems; i > 0; i--){ 
                appProjQue.items[i].remove(); }
            
            //export
            var rendObj = appProjQue.items.add(appProjAct);
            var expFile = new File(outputPath);
            rendObj.outputModule(1).file = expFile;
            // var rendMod = rendObj.outputModule(1)
            // var rendSet = rendObj.getSettings()

            // Call the native export function
            appProjQue.render();
            if(expFile.exists) ret.success = true;
            else ret.error = "Exception during export: export file is not found";
            
            ret.inPoint = prjSelLay.inPoint;
            ret.outPoint = prjSelLay.outPoint;
            ret.parentTrack = prjSelLay.index;
            ret.timebase = "secs";

            // return to the state before export
            prjSelLay.solo = saveSoloState;
            appProjAct.workAreaStart = saveArStrt;
            appProjAct.workAreaDuration = saveArDrtn;
            appProjAct.openInViewer();
        } catch (error) { ret.error = "Exception during export: " + error.toString(); }
        
        if (!ret.success) { ret.error = "Export failed - exportSequenceAE returned false"; }
        return JSON.lave(ret);
    }

    function importVideoToAE(importData) {
        ...
        try {
            ...
            var impFile = new File(importData.outputPath);
            var impItm = app.project.importFile(new ImportOptions(impFile));
            var impLay = appProjAct.layers.add(impItm);

            // var appProjAct = app.project.activeItem;
            
            impLay.startTime = importData.inPoint;
            impLay.inPoint = importData.inPoint;
            impLay.outPoint = importData.outPoint;

            ret.success = true;

        } catch (error) { ret.error = "Exception during import: " + error.toString(); }
        
        if (!ret.success) { ret.error = "Export failed - importVideoToAE returned false"; }
        return JSON.lave(ret);

    }
    
    /**
     * Get desktop path for exports
     */
    function getDetp(docs) {
        var dtpFoldPath;
        try {
            if(docs && (docs == "Documents")) dtpFoldPath = Folder.myDocuments;
            else dtpFoldPath = Folder.desktop;
            
            var appName = app.appName || "Unknown";
            // Add logging to understand path resolution
            // console.log("[getDetp] App: " + appName + ", Base path: " + dtpFoldPath.fsName);
            
            dtpFoldPath = new Folder(dtpFoldPath + "/autovfx_assets");
            // tryFold = tryFold.parent;
            if(!dtpFoldPath.exists) { 
                var created = dtpFoldPath.create(); 
                // console.log("[getDetp] Created autovfx_assets folder: " + created);
            }
            dtpFoldPath = dtpFoldPath.fsName.split("\\").join("/");
            // console.log("[getDetp] Final path: " + dtpFoldPath);
        } catch (error) {
            // Fallback to a common desktop path with autovfx_assets folder
            var fallbackDesktop = "/Users/" + $.getenv("USER") + "/Desktop";
            var fallbackFolder = new Folder(fallbackDesktop + "/autovfx_assets");
            try {
                if (!fallbackFolder.exists) {
                    fallbackFolder.create();
                }
                dtpFoldPath = fallbackFolder.fsName.split("\\").join("/");
            } catch (fallbackError) {
                // Final fallback - just use desktop
                dtpFoldPath = fallbackDesktop;
            }
        }
    ...
    function initSettings(mode, settIn){
        var setts;
        var settFile = new File(Folder.userData + '/Adobe/autovfxSetts.json');
        settFile.encoding = 'UTF-8';
        ...
        if( settIn == undefined ){
            if( !settFile.exists ) {
                ...
                settFile.open('w');
                settFile.write(setts.toSource());
                settFile.close();
            }
        
            settFile.open('r');
            setts = new Function('return ' + settFile.readln())();
            settFile.close();
        }
        else{
            settFile.open('w');
            settFile.write(settIn.toSource());
            settFile.close();
        }
        if(mode == 'getJS') return JSON.lave(setts) //setts.tranFoldIn + '_splHere^:_' + setts.projFoldIn;
        else                  return setts;
    }

    function simuDupli(inPath, outPath) {
        var tryInFile = new File(inPath);
        if(!tryInFile.exists) return("EvalScript error: inPath file is not exists.")
            
        var tryOutFile = new File(outPath);
        if(!tryOutFile.exists) tryOutFile.remove();
        
        var copyF = tryInFile.copy(tryOutFile.fsName);
        if(copyF) return(tryOutFile.fsName.split("\\").join("/"));
        else return("EvalScript simuDupli export error")
    }

    function removeAllGenerated(){
        var getSetts = initSettings();
        var getSettedFold = getDetp(getSetts.exptofold);
        var outPthFold = new Folder(getSettedFold);
        if(!outPthFold.exists) return("EvelScript error: removeAllGenerated - folder is not found");

        var rootChilds = app.project.rootItem.children;
        var getFls = outPthFold.getFiles();

        var collectItems = [];
        for(var i = 0; i< rootChilds.length; i++){         
            for(var j = 0; j< getFls.length; j++){
                if(rootChilds[i].name == getFls[j].displayName){ 
                    collectItems.push({ file: getFls[j], item:rootChilds[i], adds: [] }); 
                }
            }
        }
        
        var seqVid = app.project.activeSequence.videoTracks;
        for(var i = 0; i< collectItems.length; i++){ 
            for(var j = 0; j< seqVid.length; j++){ 
                for(var k = 0; k< seqVid[j].clips.length; k++){ 
                    if(seqVid[j].clips[k].projectItem.treePath == collectItems[i].item.treePath){ 
                        collectItems[i].adds.push(seqVid[j].clips[k]); }
                }
            }
        }

        var clearItm, seqVid = app.project.rootItem.createBin("RemoveIt")
        for(var i = 0; i< collectItems.length; i++){ 
            clearItm = collectItems[i];
            for(var j = 0; j< clearItm.adds.length; j++){ clearItm.adds[j].remove(0,0); }
            clearItm.item.moveBin(seqVid);
        }
        seqVid.deleteBin("RemoveIt");

        for(var i = 0; i< collectItems.length; i++){ collectItems[i].file.remove(); }

        return(true);
    }

    /**
     * Get sequence video format information
     */
    function getSequenceFormat() {
        try {
            if (!app.project.activeSequence) {
                return JSON.lave({ success: false, error: "No active sequence found" });
            }
            
            var sequence = app.project.activeSequence;
            var videoFormat = sequence.videoDisplayFormat;
            var frameSize = sequence.frameSizeHorizontal + "x" + sequence.frameSizeVertical;
            var frameRate = sequence.timebase;
            
            // Determine if this is 4K or higher resolution
            var is4K = sequence.frameSizeHorizontal >= 3840 || sequence.frameSizeVertical >= 2160;
            var isHD = sequence.frameSizeHorizontal >= 1920 || sequence.frameSizeVertical >= 1080;
            ...
            return JSON.lave({ success: false, error: "Error getting sequence format: " + error.toString() });
        }
    }
    ...
    function checkFileSize(filePath) {
        try {
            ...
            if (file.exists) {
                ...
                return JSON.lave({ success: true, sizeMB: sizeMB, sizeBytes: sizeBytes });
            } else {
                return JSON.lave({ success: false, error: "File not found: " + filePath });
            }
        } catch (error) {
            return JSON.lave({ success: false, error: "Error checking file size: " + error.toString() });
        }
    }

    // Public API
    return {
        exportSequenceWithPreset: exportSequenceWithPreset,
        importVideoToTimeline: importVideoToTimeline,
        exportSequenceAE: exportSequenceAE,
        importVideoToAE: importVideoToAE,
        getDetp: getDetp,
        simuDupli: simuDupli,
        removeAllGenerated: removeAllGenerated,
        initSettings: initSettings,
        checkFileSize: checkFileSize,
        getSequenceFormat: getSequenceFormat
    };
})(); 


function testJson(){ return(JSON.lave({jsontest:"succeed"})); }
...
function addJson(){
    ...
    *******************************************************************************/
    ...
    - lave(any) : Take anything and return its stringified representation,
                    more ExtendScript-friendly than JSON.lave() would be.
    ...
    of `JSON.lave()` and is entirely independent from D. Crockford's
    ...
    */
    var testRes = true;
    try{ var tstJson = JSON.lave({t:"test"}); }
    catch(e){ testRes = false; }
    if( !$.global.hasOwnProperty('JSON')||!testRes )
    {
        //==========================================================================
    ...
    try{ var tstJson = JSON.lave({t:"test"}); testRes = true; }
    catch(e){ testRes = false; }
