...
class RunwayAPI {
    ...
    async throttleRequest(requestId, requestFn) {
        return new Promise(async (resolve, reject) => {
            ...
            if (this.activeRequests.size >= this.maxConcurrentRequests) {
                console.log(`⏳ Request ${requestId} queued (${this.activeRequests.size}/${this.maxConcurrentRequests} active)`);
                this.requestQueue.push({ requestId, requestFn, resolve, reject });
                return;
            }
            ...
            console.log(`🚀 Executing request ${requestId} (${this.activeRequests.size}/${this.maxConcurrentRequests} active)`);
    ...
    processQueue() {
        if (this.requestQueue.length > 0 && this.activeRequests.size < this.maxConcurrentRequests) {
            const { requestId, requestFn, resolve, reject } = this.requestQueue.shift();
            console.log(`📤 Processing queued request ${requestId} (${this.requestQueue.length} remaining in queue)`);
    ...
    async videoToDataUri(videoFile) {
        return new Promise((resolve, reject) => {
            ...
            reader.onload = () => {
                ...
                const hasValidType = supportedTypes.some(type => dataUri.startsWith(`data:${type}`));
                
                if (!hasValidType) {
                    console.warn('⚠️ Video type may not be supported by Runway:', videoFile.type);
                }
                ...
                console.log('📊 Data URI Info:', {
                    ...
                    compressionRatio: (dataUri.length / videoFile.size).toFixed(2),
                    type: videoFile.type,
                    name: videoFile.name
                });
            ...
            reader.onerror = (error) => {
                ...
                reject(new Error(`Failed to convert video to data URI: ${error.message || 'Unknown error'}`));
            };
    ...
    async validateAsset(videoFile, dataUri) {
        ...
        if (videoFile.size > maxSizeBytes) {
            errors.push(`Video file size (${(videoFile.size / (1024 * 1024)).toFixed(2)} MB) exceeds Runway's 16MB limit`);
        }
        
        // 2. Data URI size validation
        if (dataUri && dataUri.length > 25 * 1024 * 1024) { // ~25MB for base64
            errors.push(`Data URI size (${(dataUri.length / (1024 * 1024)).toFixed(2)} MB) is too large for API transmission`);
        }
        
        // 3. File type validation
        const supportedTypes = ['video/mp4', 'video/quicktime', 'video/x-msvideo', 'video/webm'];
        if (!supportedTypes.includes(videoFile.type)) {
            warnings.push(`Video type "${videoFile.type}" may not be supported. Recommended: MP4, MOV, AVI, WebM`);
        }
        ...
        if (dataUri) {
            ...
            if (!contentTypeMatch) {
                ...
                warnings.push(`Data URI content type "${contentTypeMatch[1]}" may not be supported`);
            }
        }
            ...
            await new Promise((resolve, reject) => {
                video.onloadedmetadata = () => {
                    ...
                    if (width > 8000 || height > 8000) {
                        errors.push(`Video dimensions (${width}x${height}) exceed 8000px limit on one side`);
                    }
                    
                    // Check aspect ratio (typical limits)
                    const aspectRatio = width / height;
                    if (aspectRatio < 0.1 || aspectRatio > 10) {
                        warnings.push(`Unusual aspect ratio (${aspectRatio.toFixed(2)}), may be rejected by Runway`);
                    }
                ...
                video.onerror = () => {
                    URL.revokeObjectURL(objectUrl);
                    warnings.push('Could not read video metadata for dimension validation');
                    resolve();
                };
                video.src = objectUrl;
            });
    ...
    debugPrompt(prompt) {
        ...
        console.log('🔍 Contains potential flags:', {
            hasWeapons: /weapon|gun|knife|sword|violence|fight|attack|kill|death|blood|gore/i.test(prompt),
            hasExplicit: /sex|nude|porn|explicit|nsfw/i.test(prompt),
            hasDrugs: /drug|cocaine|heroin|marijuana|weed|alcohol|drunk/i.test(prompt),
            hasHate: /hate|racist|nazi|terrorist|bomb|suicide/i.test(prompt),
            hasCelebrity: /\b(taylor swift|elon musk|biden|trump|celebrity|famous person)\b/i.test(prompt),
            hasPersonalInfo: /email|phone|address|ssn|credit card/i.test(prompt)
        });
        console.log('💡 Suggestion: Try a more neutral, creative prompt focusing on artistic effects, colors, movement, or abstract concepts');
    }
    ...
    async generateVideo(options = {}) {
        if (!this.apiKey) {
            throw new Error('API key not set. Please configure your Runway API key.');
        }
        ...
        if (!response.ok) {
            ...
            if (response.status === 400) {
                ...
                if (errorMessage.includes('Invalid data URI') || errorMessage.includes('data URI')) {
                    console.log('❌ Data URI Error - The video data URI is malformed');
                ...
                if (errorMessage.includes('Unsupported asset type')) {
                    console.log('❌ Asset Type Error - Video format not supported');
                ...
                if (errorMessage.includes('Invalid asset dimensions')) {
                    console.log('❌ Dimension Error - Video resolution too high');
                    console.log('💡 Maximum: 8000px on either side');
                }
                
                if (errorMessage.includes('Invalid asset aspect ratio')) {
                    console.log('❌ Aspect Ratio Error - Video aspect ratio out of bounds');
                }
                
                if (errorMessage.includes('Failed to fetch asset')) {
                    console.log('❌ Fetch Error - Could not download the asset');
                    console.log('💡 This suggests a server/network issue on Runway\'s side');
                }
                
                if (errorMessage.includes('Timeout while fetching')) {
                    console.log('❌ Timeout Error - Asset download took too long');
                    console.log('💡 File may be too large or network connection slow');
                }
                ...
                if (isWindows) {
                    console.log('🪟 Windows detected - checking for platform-specific issues');
            ...
            if (errorData.error && errorData.error.includes('content moderation')) {
...
`;
        ...
        return this.pollForCompletion(result.id, undefined, undefined, onProgress);
    }
    ...
    async checkGenerationStatus(taskId) {
        ...
        for (const endpoint of endpoints) {
            try {
                ...
                if (response.ok) {
                    ...
                    const errorData = await response.json().catch(() => ({}));
                    console.log(`⚠️  Status check failed at ${endpoint}: ${response.status} - ${errorData.message || response.statusText}`);
                }
            } catch (error) {
        ...
        throw new Error(`Official Runway API status endpoints not found. Task ID: ${taskId}. This might indicate the API structure has changed.`);
    }
    ...
    async getModels() {
        ...
        return {
            success: true,
            models: [
                { id: 'gen4_aleph', name: 'Runway Gen-4 Aleph' }
            ]
        };
    }
    ...
    async retryWithBackoff(fn, maxRetries = 3, baseDelay = 2000) {
        ...
        for (let attempt = 0; attempt < maxRetries; attempt++) {
            try {
                ...
                const isTransientError = error.message.includes('Failed to fetch asset') ||
                                       error.message.includes('Timeout while fetching') ||
                                       error.message.includes('server hosting the asset may be down');
                
                if (!is400Error && !isTransientError) {
                    console.log('❌ Non-400 error, not retrying:', error.message);
                    throw error;
                }
    ...
    async processVideo(videoFile, prompt, options = {}) {
        try {
            ...
            if (validation.warnings.length > 0) {
                console.warn('⚠️ Asset validation warnings:', validation.warnings);
            ...
            if (generationResult && generationResult.id) {
                ...
                try {
                    return await this.pollForCompletion(taskId, undefined, undefined, options.onProgress);
                } catch (pollingError) {
                    console.log('⚠️  Status polling failed, but video generation was successfully initiated');
                    console.log('🎬 Official Runway API may use different status endpoints');
                    console.log('💡 Check your Runway dashboard - the video is likely being generated');
                    ...
                    return {
                        ...
                        pollingNote: 'Status polling failed - this is normal with the official Runway API structure'
                    };
                }
            } else {
                ...
                return {
                    success: true,
                    videoUrl: generationResult.videoUrl || generationResult.url || 'generation-complete',
                    status: 'completed'
                };
            }
     ...
     */
    async pollForCompletion(taskId, maxAttempts = 60, interval = 15000, onProgress) {
        let attempts = 0;
        let consecutiveErrors = 0;
        
        console.log(`🔄 Starting to poll for task: ${taskId}`);
        console.log(`Will check every ${interval/1000}s for max ${maxAttempts} attempts`);
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                const taskStatus = status.status || status.state || status.job_status || 'unknown';
                ...
                const candidates = [
                    ...
                    status.output && Array.isArray(status.output) && status.output[0] && status.output[0].progress
                ];
                ...
                if (pct == null) {
                    ...
                    if (typeof qp === 'number' && typeof qt === 'number' && qt > 0) {
                        // 5% to 30% while in queue
                        pct = 5 + (1 - clamp(qp / qt, 0, 1)) * 25;
                    } else if (taskStatus === 'IN_PROGRESS' || taskStatus === 'RUNNING' || taskStatus === 'processing') {
                        // 30%..90% over attempts while processing
                        pct = 30 + clamp(attempts / Math.max(1, maxAttempts), 0, 1) * 60;
                    } else if (taskStatus === 'queued' || taskStatus === 'PENDING') {
                        pct = 5 + clamp(attempts / Math.max(1, maxAttempts), 0, 1) * 20;
                    } else {
                        // Default gentle ramp 10..85
                        pct = 10 + clamp(attempts / Math.max(1, maxAttempts), 0, 1) * 75;
                    }
                }
                if (onProgress && typeof onProgress === 'function') {
                    try { onProgress(clamp(Math.round(pct), 0, 99), { status }); } catch (e) { /* ignore */ }
                }
                
                if (taskStatus === 'completed' || taskStatus === 'success' || taskStatus === 'COMPLETED' || taskStatus === 'SUCCEEDED') {
                    ...
                    if (status.output && Array.isArray(status.output) && status.output.length > 0) {
                        ...
                        if (typeof output === 'string') {
                            ...
                            videoUrl = output.url || output.videoUrl || output.video_url || output.downloadUrl;
                        }
                        console.log('📹 Extracted video URL from output array:', videoUrl);
                    }
                    
                    // Fallback to other possible fields
                    if (!videoUrl) {
                        videoUrl = status.videoUrl || status.output_url || status.result_url || status.video_url || status.result?.video_url;
                        console.log('📹 Using fallback video URL extraction:', videoUrl);
                    }
                ...
                if (taskStatus === 'failed' || taskStatus === 'error' || taskStatus === 'FAILED' || taskStatus === 'ERROR') {
                    throw new Error(`Generation failed: ${status.error || status.message || 'Unknown error'}`);
                }
                
                // If in progress, continue polling
                if (taskStatus === 'IN_PROGRESS' || taskStatus === 'RUNNING' || taskStatus === 'processing') {
                ...
                if (error.message.includes('status endpoints not found') && consecutiveErrors >= 3) {
                    console.log('🚨 Official Runway API endpoints not responding as expected');
                    console.log('🎬 Assuming generation completed successfully (API structure may have changed)');
                    if (onProgress) { try { onProgress(100, { note: 'assumed_complete' }); } catch(e){} }
                    return {
                        success: true,
        ...
        throw new Error(`Generation polling timed out after ${maxAttempts * interval/1000/60} minutes`);
    }
