...
class AutoVFX {
	...
	setupEventListeners() {
		...
		document
			.getElementById('versionPrevBtn')
			.addEventListener('click', () => {
				this.navigateVersion(-1)
			})

		document
			.getElementById('versionNextBtn')
			.addEventListener('click', () => {
				this.navigateVersion(1)
			})
		...
		document
			.getElementById('referenceImageBtn')
			.addEventListener('click', () => {
		...
		document
			.getElementById('referenceImageInput')
		...
		if (logoButton) {
			logoButton.forEach((elem) =>
				elem.addEventListener('click', () => {
					this.toggleAccountDashboard()
				}),
			)
		}

		const chboxExpFolds = document.querySelectorAll(
			'.chboxExpFoldCnt input',
		)
		if (chboxExpFolds) {
	...
	setupLicenseEventListeners() {
		...
		const closeInsufficientModal = document.getElementById(
		...
		const insufficientModal = document.getElementById(
	...
	async loadConfiguration() {
		...
		const getConf = await new Promise((resolve) => {
			this.csInterface.evalScript(
				'AutoVFXExtendScript.initSettings("getJS")',
				(result) => {
					if (
						...
						console.warn(
							'⚠️ initSettings() failed, using fallback default settings',
						)
						resolve(false)
					} else {
	...
	async handleExport() {
		try {
			...
			console.log(
				'🧹 EXPORT: Clearing existing video versions before new export',
			)
			...
			if (exportData) {
				...
				console.log(
					'📦 EXPORT: Original video stored in currentExportedVideo, not adding to videoVersions',
				)
	...
	async exportVideoSegment() {
		try {
			...
			const sequenceFormat = await new Promise((resolve) => {
				this.csInterface.evalScript(
					'AutoVFXExtendScript.getSequenceFormat()',
					(result) => {
						try {
							resolve(JSON.parse(result))
						} catch (e) {
							resolve({
								success: false,
								error: 'Failed to parse sequence format',
							})
						}
					},
				)
			...
			if (sequenceFormat.success) {
				if (sequenceFormat.is4K) {
					...
					console.log(
						`📺 Detected 4K sequence (${sequenceFormat.frameSize}) - using 4K optimized preset`,
					)
				} else if (sequenceFormat.isHD) {
					presetType = 'HD optimized'
					console.log(
						`📺 Detected HD sequence (${sequenceFormat.frameSize}) - using HD optimized preset`,
					)
				} else {
					console.log(
						`📺 Detected sequence (${sequenceFormat.frameSize}) - using standard preset`,
					)
				}
			}
			...
			this.updateProgress(
				'progressSection',
				0,
				`Exporting with ${presetType} preset...`,
			)
			...
			const dePth = await new Promise((resolve) => {
				this.csInterface.evalScript(
					'AutoVFXExtendScript.getDetp("' +
						this.userConfig.exptofold +
						'")',
					(result) => {
						if (
							result === 'EvalScript error.' ||
							...
							console.warn(
								'⚠️ getDetp() failed, using fallback desktop path with autovfx_assets folder',
							)
							const fallbackPath =
								'/Users/' +
								(process.env.USER || 'user') +
								'/Desktop/autovfx_assets'
							...
							try {
								...
								console.warn(
									'⚠️ Could not create autovfx_assets folder:',
									e,
								)
							}
			...
			const exportResult = await new Promise((resolve, reject) => {
				...
				if (hostapp == 'AEFT')
					script = `AutoVFXExtendScript.exportSequenceAE("${outputPath}")`
					...
					try {
						...
						if (!expRes.success) {
							if (expRes && expRes.error) {
								return reject(new Error(expRes.error))
							}
							console.warn(
							...
							return reject(
								new Error(
									'Timeline info unavailable. Please open a sequence in Premiere and try again.',
								),
							)
	...
	async validateExportFileSize(filePath) {
		try {
			const sizeCheckResult = await new Promise((resolve) => {
				this.csInterface.evalScript(
					`AutoVFXExtendScript.checkFileSize("${filePath}")`,
					(result) => {
						try {
							resolve(JSON.parse(result))
						} catch (e) {
			...
			if (sizeCheckResult.success) {
				...
				if (sizeMB > maxSize) {
					...
					console.log(
						`✅ Export file size (${sizeMB.toFixed(2)} MB) is within the 12MB processing limit`,
					)
				...
				console.warn(
					'⚠️ Could not verify file size:',
					sizeCheckResult.error,
				)
			}
		} catch (error) {
	...
	async handleGenerate() {
		...
		if (
			...
			if (versionData && versionData.path) {
				...
				console.log(
					`🎯 Using selected video source: ${this.selectedVideoSource}`,
				)
			}
		...
		if (!sourceVideoPath) {
			this.showError(
				'No video source available. Please export a video first or select a valid video version.',
			)
			return
		}
		...
		try {
			...
			const generatedVideoUrl = await this.generateWithRunway(
				sourceVideoPath,
				prompt,
				(pct) => {
			...
			if (generatedVideoUrl) {
				...
				if (
					generatedVideoUrl !== 'runway-generation-initiated' &&
					generatedVideoUrl !== 'runway-generation-completed-no-url'
				) {
					const versionId = `v${Date.now()}`
					const promptText =
						document.getElementById('promptInput')?.value || ''
					this.addVideoVersion(
						versionId,
						generatedVideoUrl,
						promptText,
					)
	...
	async handleImport() {
		try {
			...
			if (
				...
				if (!this.currentExportedVideo) {
					throw new Error(
						'No original export available to import. Please export first.',
					)
				}
				importPath = this.currentExportedVideo
			} else {
				const versionData = this.videoVersions.get(
					this.selectedVideoSource,
				)
				if (!versionData || !versionData.path) {
			...
			if (!fs.existsSync(importPath)) {
				throw new Error(
					`Import file does not exist: ${importPath}. Check if download completed successfully.`,
				)
			}
			...
			try {
				...
				console.log(
					`✅ Import file verified: ${(stats.size / 1024 / 1024).toFixed(2)} MB`,
				)
			} catch (accessError) {
				throw new Error(
					`Import file exists but is not readable: ${accessError.message}`,
				)
			}
			...
			let successMessage
			const positionMsg =
				this.lastExportPosition &&
				this.lastExportPosition.insertionTime !== undefined
	...
	async importToTimeline(importData) {
		return new Promise((resolve, reject) => {
			let hostapp = this.csInterface.hostEnvironment.appId //PPRO or AEFT
			let script =
				'AutoVFXExtendScript.importVideoToTimeline(' +
				JSON.stringify(importData) +
				')'
			if (hostapp == 'AEFT')
				script =
					'AutoVFXExtendScript.importVideoToAE(' +
					JSON.stringify(importData) +
					')'

			console.log(script)
			console.log(
				'🎬 Importing video to timeline with position: ' +
					importData.inPoint / importData.timebase +
					' secs',
			)
			var importResult
			...
			this.csInterface.evalScript(script, (result) => {
				...
				try {
					...
					console.log(
						'✅ Position-aware import success:',
						importResult,
					)
				} catch (error) {
					console.error(
						'❌ Position-aware import result parsing failed:',
						error,
					)
				}
			})
	...
	showVersionSelector() {
		...
		const selectorResult = document.getElementById(
			'videoVersionSelectorResult',
		)
		if (selectorResult) {
	...
	updateVersionSlider() {
		...
		// Result view mirrors
		const currentVersionTextResult = document.getElementById(
			'currentVersionTextResult',
		)
			...
			if (
				...
				versions.push({
					...
					description: videoData.prompt
						? videoData.prompt.substring(0, 40) + '...'
						: 'AI Generated',
				})
				versionCount++
			}
		...
		if (currentVersionText)
			currentVersionText.textContent = currentVersion.text
		if (currentVersionTextResult)
			currentVersionTextResult.textContent = currentVersion.text
		...
		if (
			resultView &&
			resultVideoEl &&
			resultView.classList.contains('active')
		) {
			let previewSrc = ''
			if (this.selectedVideoSource === 'original') {
				if (this.currentExportedVideo) {
					previewSrc = this.currentExportedVideo.startsWith('file://')
						? this.currentExportedVideo
						: `file://${this.currentExportedVideo}`
				}
			} else {
			...
			if (previewSrc) {
				...
				resultVideoEl.addEventListener(
					'loadeddata',
					() => {
		...
		console.log(
			`📹 Version Selector: ${currentVersion.text} (${currentVersion.id}) - ${versions.length} total versions available`,
		)
	}
	...
	navigateVersion(direction) {
		...
		const totalVersions =
			(this.currentExportedVideo ? 1 : 0) +
			Array.from(this.videoVersions.values()).filter(
				(v) =>
					v.path &&
					v.path !== 'runway-generation-initiated' &&
					v.path !== 'runway-generation-completed-no-url',
			).length
		...
		if (this.currentVersionIndex >= totalVersions)
			this.currentVersionIndex = totalVersions - 1
	...
	addVideoVersion(versionId, videoPath, prompt = '') {
		...
		console.log(
			`📦 Added video version: ${versionId} (${this.videoVersions.size} total versions)`,
		)
		...
		this.currentVersionIndex = allVersions.findIndex(
			(id) => id === versionId,
		)
		...
		console.log(
			`📹 Auto-selected new version: ${versionId} at index ${this.currentVersionIndex}`,
		)

		// Update version selector if it's visible
		if (
			document.getElementById('videoVersionSelector') &&
			!document
				.getElementById('videoVersionSelector')
	...
	async startExtendedPolling(taskId) {
		...
		const pollForVideo = async () => {
			try {
				attempts++
				console.log(
					`📡 Extended polling attempt ${attempts}/${maxAttempts}...`,
				)

				// Log progress for extended polling
				if (attempts === 1) {
					console.log(
						'🎬 Extended polling: Initial generation check...',
					)
				} else if (attempts < 10) {
					console.log(
						`🎬 Extended polling: Checking generation progress (attempt ${attempts})...`,
					)
				}
				...
				if (status) {
					...
					if (
						...
						if (videoUrl) {
							console.log(
								'🎉 Extended polling SUCCESS! Video is ready:',
								videoUrl,
							)
							this.showError(
								`🎉 Your video is ready! Displaying now...`,
								'success',
							)
							...
							await this.settleCreditsOnCompletion(
								...
								true,
								10,
							) // Assume 10 seconds actual duration
							...
							console.log(
								`📦 Added generated video to versions: ${versionId}`,
							)
							...
							console.log(
								'⚠️  Status is complete but no video URL found',
							)
							this.showError(
								`✅ Video completed! Task ID: ${taskId}. Check your account dashboard.`,
								'success',
							)
						...
						console.log(
							'❌ Extended polling: Video generation failed',
						)
						this.showError(
							`❌ Video generation failed. Task ID: ${taskId}`,
							'error',
						)
						...
						console.log(
							`⏳ Extended polling: Still generating... ${Math.round(progress * 100)}%`,
						)
						...
						if (progress > 0) {
							...
							if (progressPercent < 30)
								progressPercent = 30 + attempts * 2 // Show incremental progress
							if (progressPercent > 95) progressPercent = 95 // Don't show 100% until complete

							console.log(
								`🎬 Extended polling progress: ${progressPercent}% - ${message}`,
							)
						}
				...
				if (attempts < maxAttempts) {
					...
					this.showError(
						`⏰ Generation timeout reached. Task ID: ${taskId}. Check your account dashboard.`,
						'warning',
					)
				}
			} catch (error) {
		...
		try {
			...
			if (
				...
				if (
					...
					console.log(
						`🔄 Using generated video version: ${this.selectedVideoSource}`,
					)
					sourceVideoPath = versionData.path
					...
					console.log(
						'📹 Generated video not found, using original exported video as source',
					)
				}
			} else {
			...
			if (
				...
				console.log(
					`✅ Successfully read local video file (${(videoBuffer.length / (1024 * 1024)).toFixed(2)} MB)`,
				)
			}
			...
			if (videoDuration < 1.0) {
				throw new Error(
					`Video duration (${videoDuration.toFixed(2)}s) is too short. Video processing requires clips to be at least 1 second long. Please select a longer clip or extend your clip in the timeline.`,
				)
			}

			console.log(
				`✅ Video duration validation passed: ${videoDuration.toFixed(2)}s`,
			)
	...
	async downloadVideo(videoUrl) {
		try {
			...
			const blob = await response.blob()
			console.log(
				`📦 Video downloaded: ${(blob.size / 1024 / 1024).toFixed(2)} MB`,
			)
			...
			if (this.currentExportedVideo) {
				...
				localPath = path.join(
					exportDir,
					`autovfx_generated_${timestamp}.mp4`,
				)
			...
			if (fs.existsSync(localPath)) {
				...
				console.log(
					`✅ Video saved successfully: ${localPath} (${(stats.size / 1024 / 1024).toFixed(2)} MB)`,
				)
				return localPath
			} else {
	...
	displayResult(videoUrl) {
		...
		if (!videoUrl.startsWith('http') && !videoUrl.startsWith('blob:')) {
			console.log(
				'🔧 Local file detected, using blob URL for CEP compatibility',
			)
			this.loadVideoWithBlobUrl(videoUrl, video)
	...
	async loadVideoWithBlobUrl(videoPath, videoElement) {
		try {
			console.log(
				'🔧 Loading video with blob URL for CEP compatibility...',
			)
	...
	handleVideoLayout(videoElement) {
		try {
			...
			if (isVertical) {
				console.log(
					'📱 Detected vertical video:',
					`${videoWidth}x${videoHeight}`,
				)
				container.classList.add('vertical')
			} else {
				console.log(
					'🖥️ Detected horizontal video:',
					`${videoWidth}x${videoHeight}`,
				)
	...
	initVideoControls() {
		...
		if (!video) {
			console.log(
				'❌ Video element not found, skipping video controls initialization',
			)
			return
		}
		...
		if (!progressBar || !progressFill) {
			console.log(
				'❌ Required video control elements not found, skipping initialization',
			)
	...
	toggleChboxExpFolds(elemClick) {
		...
		let findEl
		if (elemClick.checked)
			findEl = elemClick.id == 'switchFold' ? 'Documents' : 'Yes'
		else findEl = elemClick.id == 'switchFold' ? 'Desktop' : 'No'

		findEl = findTexts.find((el) => el.textContent === findEl)
		findTexts.forEach((elem) => {
			elem.classList.remove('activeChBox')
		})
		findEl.classList.add('activeChBox')

		// let getConf = { 'exptofold': "Desktop", 'clearExp': false };
		// const getConf = await new Promise((resolve) => {
		if (elemClick.id == 'switchFold')
			this.userConfig.exptofold = elemClick.checked
				? 'Documents'
				: 'Desktop'
		...
		this.csInterface.evalScript(
			...
			(result) => {
				if (
					...
					console.warn(
						'⚠️ initSettings() save changes failed, using fallback default settings',
					)
				}
			},
		)
		...
		try {
			...
			if (success) {
				...
				const refundCredits = Math.max(
					0,
					reservedCredits - actualCreditsUsed,
				)
				...
				this.showError(
					`✅ Generation completed! Used ${actualCreditsUsed} credits.`,
					'success',
				)
			} else {
				// Generation failed - refund all reserved credits
				const refundCredits =
					this.currentReservation?.reservedCredits || 0
				...
				this.showError(
					`❌ Generation failed. ${refundCredits} credits refunded.`,
					'error',
				)
			}
			...
			console.log(
				'⚠️  Credit settlement will be retried by the backend system',
			)
		}
	}
}
