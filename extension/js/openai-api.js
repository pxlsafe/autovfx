...
class OpenAIAPI {
	 ...
	 */
	async pollResponsesResult(
		responseId,
		{ intervalMs = 600, timeoutMs = 10000 } = {},
	) {
		...
		while (Date.now() - start < timeoutMs) {
			...
			if (!res.ok) {
				...
				throw new Error(
					`OpenAI Responses API poll error: ${res.status} - ${err.error?.message || 'Unknown error'}`,
				)
			}
			...
			if (data.status === 'failed' || data.status === 'cancelled') {
				throw new Error(
					`Responses API returned terminal status: ${data.status}`,
				)
			}
...
Enhance this prompt for Runway ML video generation: "${userPrompt}"`
		...
		try {
			...
			if (!response.ok) {
				...
				throw new Error(
					`OpenAI Responses API error: ${response.status} - ${errorData.error?.message || 'Unknown error'}`,
				)
			}
			...
			if (initial.status === 'incomplete' && initial.id) {
				try {
					data = await this.pollResponsesResult(initial.id, {
						intervalMs: 700,
						timeoutMs: 12000,
					})
					...
					console.warn(
						'⚠️ Responses polling failed:',
						pollErr.message,
					)
					// Try to extract partial text if any in the initial payload
					const partialText = this.extractTextFromResponsesOutput(
						initial.output,
					)
					if (partialText) {
						const cleanedPartial = partialText.replace(
							/^["']|["']$/g,
							'',
						)
						return {
			...
			if (!data.output_text && !data.output) {
				throw new Error(
					'Invalid response format from OpenAI Responses API',
				)
			}
			...
			if (data.output_text) {
				...
				enhancedPrompt = this.extractTextFromResponsesOutput(
					data.output,
				)
			}
		...
		if (this.model === 'gpt-5') {
			try {
				...
				if (
					result.success &&
					result.enhanced &&
					result.enhanced.trim().length > 0
				) {
					return result
				}
				console.log(
					'🔄 GPT-5 responses API returned empty, falling back to chat completions...',
				)
			} catch (error) {
				console.log(
					'🔄 GPT-5 responses API failed, falling back to chat completions:',
					error.message,
				)
			}
		}
...
- Do NOT return quotes, explanations, or multiple options. Return only the prompt text.`
		...
		try {
			...
			console.log(
				'💬 Falling back to chat completions with model:',
				modelForChat,
			)
			...
			const response = await fetch(`${this.baseUrl}/chat/completions`, {
				...
				body: JSON.stringify({
					...
					max_completion_tokens:
						this.model === 'gpt-5' ? 500 : this.maxTokens,
				}),
			})
