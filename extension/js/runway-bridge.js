...
class RunwayMLBridge {
	...
	async createVideoToVideoTask(params) {
		...
		if (promptVideo instanceof File || promptVideo instanceof Blob) {
			...
			typeof promptVideo === 'string' &&
			promptVideo.startsWith('data:')
		) {
		...
		if (!taskResponse.ok) {
			...
			throw new Error(
				`Task creation failed: ${taskResponse.status} ${errorText}`,
			)
		}
	...
	async uploadVideoAsset(videoBlob) {
		...
		if (!response.ok) {
			...
			throw new Error(
				`Asset upload failed: ${response.status} ${errorText}`,
			)
		}
	...
	async waitForTaskOutput(taskId) {
		...
		for (let attempt = 0; attempt < maxAttempts; attempt++) {
			...
			if (task.status === 'FAILED') {
				throw new Error(
					`Task failed: ${task.failure?.reason || 'Unknown error'}`,
				)
			}
